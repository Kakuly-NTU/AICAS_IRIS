...
class HashPallette():
    def __init__(self, tech):
        ...
        self.logic_lut = {} # this stores logic label colors
        ...
        self.label_color = {
            'ff': [0, 255, 255], # blue
            'logic': [60, 255, 255], # green
            'fill': [120, 255, 255], # red
            'other': [90, 255, 255] # yellow
        }

        self.logic_color = {
            'ff': [0, 255, 255], # blue
            'logic': [0, 255, 255], # blue
            'fill': [120, 255, 255], # red
            'other': [90, 255, 255] # yellow
        }
    ...
    def name_to_logic_label(self, name, orientation):
        if name in self.logic_lut:
            return self.logic_lut[name]
        else:
            label_name = self.tech.map_name_to_label(name)
            hsv_color = self.logic_color[label_name]

            rgb_color = cv2.cvtColor(np.array([[hsv_color]], dtype=np.uint8), cv2.COLOR_HSV2RGB)[0][0]
            self.logic_lut[name] = (float(rgb_color[0]), float(rgb_color[1]), float(rgb_color[2]))
            return self.logic_lut[name]
