...
def export_png(cells, interactive=False):
    ...
    progress = progressbar.ProgressBar(min_value = 0, max_value=total_steps, prefix = f'Extracting {gds_file.stem} ')
    step = 0
    ...
    cv2.imwrite(str(image_directory / ('gds_to_png_images/' + gds_file.stem + '_airy_' + '.png')), airy)
    cv2.imwrite(str(image_directory / ('gds_to_png_images/' + gds_file.stem + '.png')), image)
...
def export_lib(cell_list, interactive=False):
    ...
    progress = progressbar.ProgressBar(min_value = 0, max_value=len(boxes), prefix = f'Library mapping {gds_file.stem} ')
    ...
    if interactive:
        ...
        cv2.imwrite(str(image_directory / ('coloured_images/' + gds_file.stem + '_lib.png')), image)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
    
    cv2.imwrite(str(image_directory / ('coloured_images/' + gds_file.stem + '_lib.png')), image)
    with open(str(image_directory / ('coloured_images/' + gds_file.stem + '_lib.json')), 'w') as f:
        json.dump(export, f)

    with open(str(image_directory / ('coloured_images/' + gds_file.stem + '_relative_lib.json')), 'w') as f:
...
def export_label(cell_list, interactive=False):
    ...
    if interactive:
        cv2.imshow(f'{gds_file.stem} library', image)
        cv2.imwrite(str(image_directory / ('labelled_images/' + gds_file.stem + '_label.png')), image)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
    
    cv2.imwrite(str(image_directory / ('labelled_images/' + gds_file.stem + '_label.png')), image)
    with open(str(image_directory / ('labelled_images/' + gds_file.stem + '_label.json')), 'w') as f:
        json.dump(export, f)

    with open(str(image_directory / ('labelled_images/' + gds_file.stem + '_relative_label.json')), 'w') as f:
        json.dump(export_relative, f)


def export_logic_label(cell_list, interactive=False):
    boxes = []
    export = {}
    export_relative = {}
    for ref in cell_list.references:
        color = tm.pallette.name_to_logic_label(ref.ref_cell.name, map_orientation(ref.rotation, ref.x_reflection))
        color_int = list(map(int, color))
        if ref.get_bounding_box() is not None:
            boxes += [(ref.get_bounding_box(), color_int, ref.ref_cell.name)]
    ...
    if interactive:
        cv2.imshow(f'{gds_file.stem} library', image)
        cv2.imwrite(str(image_directory / ('logic_images/' + gds_file.stem + '_logic.png')), image)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
    
    cv2.imwrite(str(image_directory / ('logic_images/' + gds_file.stem + '_logic.png')), image)
    with open(str(image_directory / ('logic_images/' + gds_file.stem + '_logic.json')), 'w') as f:
        json.dump(export, f)

    with open(str(image_directory / ('logic_images/' + gds_file.stem + '_relative_logic.json')), 'w') as f:
        json.dump(export_relative, f)



def export_one_class_label(cell_list, cell_type, interactive=False):
    boxes = []
    export = {}
    export_relative = {}
    for ref in cell_list.references:
    ...
    progress = progressbar.ProgressBar(min_value = 0, max_value=len(boxes), prefix = f'Library mapping {gds_file.stem} ')
    for (i, (rect, color, name)) in enumerate(boxes):
        actual_cell_type = tm.map_name_to_label(name)
        if str(actual_cell_type) != str(cell_type):
            continue
        r = np.rint(rect * PIX_PER_UM).astype(int)
        cv2.rectangle(image, r[0] - offset, r[1] - offset, color, thickness=-1, lineType=cv2.LINE_8)
        progress.update(i)
